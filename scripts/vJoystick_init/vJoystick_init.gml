...
function vJoystick_init(){
	...
	*/
	...
	//Landscape
	global.scrMiddle.x0 = display_get_width()/2;
	global.scrMiddle.x1 = display_get_width()/2;
	global.scrMiddle.y0 = 0;
	global.scrMiddle.y1 = display_get_height();
	
	// Get the left and right screen zones
	global.sZ.l.sX = 0;							// Left zone start
	global.sZ.l.eX = display_get_width()/2-1;	// Left zone end
	global.sZ.r.sX = display_get_width()/2+1;	// Right zone start
	global.sZ.r.eX = display_get_width();		// right zone end
	// y-axis limits
	global.minY = 0;							// Screen top (y-axis)
	global.maxY = display_get_height();			// Screen end (y-axis)
